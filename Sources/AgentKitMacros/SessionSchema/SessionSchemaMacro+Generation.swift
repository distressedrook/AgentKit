...
extension SessionSchemaMacro {
  ...
  static func generateDecodedToolRunEnum(
    ...
    let makeUnknownDeclaration =
      "  " + accessModifier + " static func makeUnknown(toolCall: AgentKit.Transcript.ToolCall) -> Self {"
    ...
    let declaration =
      accessModifier + " enum DecodedToolRun: AgentKit.DecodedToolRun, @unchecked Sendable {"
  ...
  static func generateDecodedGroundingType(
    ...
    guard !groundings.isEmpty else {
      let declaration =
        accessModifier + " struct DecodedGrounding: AgentKit.DecodedGrounding, @unchecked Sendable {}"
    ...
    let declaration =
      accessModifier + " enum DecodedGrounding: AgentKit.DecodedGrounding, @unchecked Sendable {"
  ...
  static func generateStructuredOutputsFunction(
    ...
    let signature =
      accessModifier
        + " static func structuredOutputs() -> [any (AgentKit.DecodableStructuredOutput<DecodedStructuredOutput>).Type]"
  ...
  static func generateDecodedStructuredOutputEnum(
    ...
    if !outputs.isEmpty {
      let cases = outputs
        .map { output in
          ...
          return "  case \(caseName)(AgentKit.StructuredOutputSnapshot<\(output.typeName)>)"
        }
        .joined(separator: "\n")
      sections.append(cases)
    }

    sections.append("  case unknown(AgentKit.Transcript.StructuredSegment)")
    sections.append("")
    let makeUnknownDeclaration =
      "  " + accessModifier + " static func makeUnknown(segment: AgentKit.Transcript.StructuredSegment) -> Self {"
    ...
    let declaration =
      accessModifier + " enum DecodedStructuredOutput: AgentKit.DecodedStructuredOutput, @unchecked Sendable {"
  ...
  static func generateDecodableStructuredOutputTypes(
    ...
    outputs.map { output -> DeclSyntax in
      ...
      return
        """
        private struct \(raw: resolvableName): AgentKit.DecodableStructuredOutput, @unchecked Sendable {
          typealias Base = \(raw: schemaType)

          static func decode(
            _ structuredOutput: AgentKit.StructuredOutputSnapshot<\(raw: output.typeName)>

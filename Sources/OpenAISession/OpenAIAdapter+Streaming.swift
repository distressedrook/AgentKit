...
extension OpenAIAdapter {
  ...
  private func handleOutputItemAdded(
    ...
    case let .outputMessage(message):
      let status: AgentKit.Transcript.Status = transcriptStatusForMessage(message.status)
...
private struct StreamingMessageState {
  ...
  var status: AgentKit.Transcript.Status
...
) -> AgentKit.Transcript.Status {
  switch status {
  case .completed:
    AgentKit.Transcript.Status.completed
  case .incomplete:
    AgentKit.Transcript.Status.incomplete
  case .inProgress:
    AgentKit.Transcript.Status.inProgress
  }
}
...
private func transcriptStatusForFunctionCall(
  ...
  switch status {
  case .completed:
    return AgentKit.Transcript.Status.completed
  case .incomplete:
    return AgentKit.Transcript.Status.incomplete
  case .inProgress:
    return AgentKit.Transcript.Status.inProgress
  }
}

...
/// High-level client for working with Anthropic's Messages API using AgentKit.
///
...
>: LanguageModelProvider, @unchecked Sendable {
  ...
  @ObservationIgnored public var tools: [any AgentKitTool] {
    adapter.tools
  }

  /// Transcript of the session, including prompts, tool calls, and model outputs.
  public var transcript: AgentKit.Transcript = Transcript()
  ...
  public init<each ToolType>(
    ...
    var wrappedTools: [any AgentKitTool] = []
    _ = (repeat wrappedTools.append(_AgentKitToolWrapper(tool: each tools)))

...
AgentKit simplifies AI agent development by providing a clean, intuitive API that handles all the complexity of agent loops, tool execution, and adapter communication. Inspired by Apple's FoundationModels framework, it brings the same elegant, declarative approach to cross-platform AI agent development.
...
- Build SDK
  - `xcodebuild -workspace AgentKit.xcworkspace -scheme ExampleApp -destination "platform=iOS Simulator,name=iPhone 17 Pro,OS=latest" build -quiet`
- Build AgentRecorder (CLI)
  - `xcodebuild -workspace AgentKit.xcworkspace -scheme AgentRecorder -destination "platform=macOS" build -quiet`
- Build Tests
  - `xcodebuild -workspace AgentKit.xcworkspace -scheme AgentKitTests build -quiet`
- Run Tests
  - `xcodebuild -workspace AgentKit.xcworkspace -scheme AgentKitTests -testPlan AgentKitTests test -quiet`
...
xcodebuild -workspace AgentKit.xcworkspace -scheme AgentRecorder -destination "platform=macOS,arch=arm64" -derivedDataPath .tmp/DerivedData build -quiet
